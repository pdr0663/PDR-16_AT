param(
    ...
    [int]$PromptTimeoutMs = 1000,
    [int]$QuietDrainMs = 120,
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        if ($completed -eq 0) {
            ...
            if ($read -gt 0) {
                ...
                [void](Write-Host -NoNewline $chunk)
                $quietDeadline = [DateTime]::UtcNow.AddMilliseconds($QuietDrainMs)
                continue
            }
        }

        if ([DateTime]::UtcNow -ge $quietDeadline) {
            break
        }
    }
...
try {
    ...
    $startup = Drain-PipeOutput -Pipe $pipe -PromptTimeoutMs $PromptTimeoutMs -QuietDrainMs $QuietDrainMs
    if ($startup.Length -gt 0) {
        Write-TranscriptLine -Buffer $transcript -Text $startup
    }
    ...
    foreach ($sourceName in $sourceFiles) {
        ...
        for ($index = 0; $index -lt $lines.Length; $index++) {
            ...
            $response = Drain-PipeOutput -Pipe $pipe -PromptTimeoutMs $PromptTimeoutMs -QuietDrainMs $QuietDrainMs
            if ($response.Length -gt 0) {
                Write-TranscriptLine -Buffer $transcript -Text $response
            }
    ...
    if ($ProbeWords) {
        ...
        $probe = Drain-PipeOutput -Pipe $pipe -PromptTimeoutMs $PromptTimeoutMs -QuietDrainMs $QuietDrainMs
        if ($probe.Length -gt 0) {
            Write-TranscriptLine -Buffer $transcript -Text $probe
        }
    }
}
finally {
